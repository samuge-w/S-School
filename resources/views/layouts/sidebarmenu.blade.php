...
<div class="logo">
  <a  class="js-arrow" href="#">
    @php
      $institute = getinstitueinfo();
    @endphp
    @if($institute && $institute->logo)
      <img src="{{ asset('assets/images/schools/' . $institute->logo) }}" alt="{{ $institute->name }}" style="max-height: 60px;" />
    @elseif(Session::get('inName')!='')
      <h2>{{Session::get('inName')}}</h2>
    @else
      <img src="images/icon/logo-white.png" alt="School Logo" />
    @endif
    </a>
</div>
