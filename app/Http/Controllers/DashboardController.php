	...
	public function index(Request $request)
	{
		...
		$book      = AddBook::count();
 		$total     = [
 	     ...
 	     // 	//graph data
 	//   dd($tstudent);
        // Cross-database month/year expressions (SQLite vs MySQL/PostgreSQL)
        $driver = DB::getDriverName();
        $monthExpr = $driver === 'sqlite' ? "CAST(strftime('%m', date) AS INTEGER)" : 'month(date)';
        $yearExpr  = $driver === 'sqlite' ? "CAST(strftime('%Y', date) AS INTEGER)" : 'year(date)';

        $monthlyIncome = Accounting::selectRaw("$monthExpr as month, sum(amount) as amount, $yearExpr as year")
                        ->where('type','Income')
                        ->groupBy('month','year')
                        ->get();
//,DB::RAW('IFNULL(sum(payableAmount),0) as payTotal,IFNULL(sum(paidAmount),0) as paiTotal,(IFNULL(sum(payableAmount),0)- IFNULL(sum(paidAmount),0)) as dueamount')
        $stdBillYearExpr = $driver === 'sqlite' ? "CAST(strftime('%Y', stdBill.created_at) AS INTEGER)" : 'year(stdBill.created_at)';
        $tutionfees = FeeCol::join('billHistory','stdBill.billNo','=','billHistory.billNo')->select(DB::RAW("billHistory.month, $stdBillYearExpr as year,sum(stdBill.payableAmount) as payTotal,IFNULL(sum(paidAmount),0) as paiTotal,(IFNULL(sum(payableAmount),0)- IFNULL(sum(paidAmount),0)) as dueamount"))
        ...
        $monthlyExpences = Accounting::selectRaw("$monthExpr as month, sum(amount) as amount, $yearExpr as year")
                                    ->where('type','Expence')
                                    ->groupBy('month','year')
                                    ->get();
